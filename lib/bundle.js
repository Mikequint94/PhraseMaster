...
class Translator {
  ...
  static translate(string, language) {
    ...
    $.ajax({
      ...
      translations[data.data.translations[0].translatedText.slice(-1)] = (data.data.translations[0].translatedText.slice(0, -2).replaceAll("&#39;", "'"));
    });
  ...
  round() {
    ...
    checkResponse(input) {
      ...
      if (inputLowerNoAccent === phraseLowerNoAccent) {
        ...
        document.getElementById('textinput').className = "phase3input-fail";
        this.redtext = true;
        let myThis = this;
        setTimeout(function() {
          document.getElementById('textinput').className = "phase3input";
          myThis.redtext = false;
        }, 800);
      }
    }
  //
