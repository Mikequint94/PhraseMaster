...
document.addEventListener("DOMContentLoaded", function(){
  ...
  nextButton.addEventListener("click", (e) => {
    ...
    if (phasenum === 0) {
      ...
      phaseTwo.start(ctx, translations, fulltarget);
      phasenum +=1;
      phaseTwo.instructions(ctx);
    } else if (phasenum === 3) {
      phaseTwo.game(ctx, translations, fulltarget);
    }
  });
  ...
  new __WEBPACK_IMPORTED_MODULE_1__game_view___default.a(game, ctx).start();
    __WEBPACK_IMPORTED_MODULE_1__game_view___default.a.drawStars(ctx, fillnum);
  const phaseTwo = new __WEBPACK_IMPORTED_MODULE_2__phase2___default.a(ctx, game);
